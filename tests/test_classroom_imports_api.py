...
def test_parse_classroom_students_xlsx() -> None:
    content = build_xlsx(
        [
            [" full_name ", " EMAIL "],
            [" Alex Student ", " Alex.Student@Example.COM "],
            ["Maria Student", "maria.student@example.com"],
        ]
    )
...
def test_rejects_duplicate_emails_case_insensitively() -> None:
    ...
    assert error_info.value.issues[0].row_number == 3
    assert error_info.value.issues[0].column == "email"
    assert error_info.value.issues[0].message == "Email duplicates row 2"
...
def test_rejects_missing_required_header() -> None:
    ...
    assert error_info.value.issues[0].row_number == 1
    assert error_info.value.issues[0].column == "email"
    assert error_info.value.issues[0].message == "Required header is missing"
...
def test_rejects_formulas() -> None:
    content = build_xlsx(
        [
            ["full_name", "email"],
            ['=CONCAT("Alex", " Student")', "alex@example.com"],
        ]
    )
    ...
    assert error_info.value.issues[0].row_number == 2
    assert error_info.value.issues[0].column == "full_name"
    assert error_info.value.issues[0].message == "Formulas are not allowed"
